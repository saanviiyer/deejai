...
Set a key before launch so the plain-language producer understands any phrasing
(the offline parser knows only a keyword set). Without a key, it uses the offline
rule-based parser and says so:
...
- **Import your own takes**: "import takes", or drag WAVs onto the track area.
  ...
  where it fits** and does not force them all to start together. A take that
...
- **Align & tune** (the **align** button): re-positions your takes by matching
  ...
  marker appears). That marker overrides detection. The **pitch correct** slider applies
...
- **Software instruments + MIDI**: "+ inst" adds a synth track (synth / keys /
  ...
  **tied** noteheads joined by a slur. Consecutive eighths and sixteenths inside
  ...
  notes. Play notes live from your **computer keyboard** (A to K, Z/X
...
- **Save / open**: "save" downloads a `.deejai.json` (source takes + engine
  ...
  The mix, effects, instruments, and the alignment (snap resolution, timing
  reference, manual downbeat, and pitch-correction amount) are re-applied, so the
  placements and tuned audio come back as you left them. Bounced-audio
  tracks and comp takes are embedded in the file too (gzip-compressed losslessly,
  and a drum bounce shrinks ~4x), so they survive a reopen.
...
- **Metronome + count-in + time signature**: **click** toggles the metronome,
  ...
  **time signature**, with numerator **and** denominator (e.g. 6/8, 3/4, 5/8). The beat
...
- **On-screen keyboard**: a playable piano at the bottom of the piano roll.
  ...
  playing is recorded (alongside the A to K computer keys and any MIDI keyboard).
...
- **Export**: **export wav** renders the graph **as you hear it** to a WAV in the
  browser. Your volume, pan, mute/solo, nudge, effects, and master bus are
  included. **share** renders the same mix to a small compressed file
